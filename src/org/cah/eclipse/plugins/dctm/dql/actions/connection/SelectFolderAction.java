...
/**
 * <H4>Select a content of a folder or a cabinet.</H4>
 * <DL>
 * <DT><B>Copyright : </B>
 * <DD>(c) 2004 Christopher Harper</DD>
 * </DT>
 * <DT><B>Created : </B>
 * <DD>Dec 23, 2004 2:00:50 PM.</DD>
 * </DT>
 * </DL>
 * <p>
 ...
 * @since 1.0.1
 */
public class SelectFolderAction
								extends
									AbstractBaseAction
{
	...
	public void run()
	{
		...
		{
			final FolderModel folderModel = (FolderModel) selectedModel;
			...
			try
			{
				...
				final StringBuffer attributeDQL = new StringBuffer(
					"select distinct * , ").append(ISysObject.R_LOCK_OWNER) //$NON-NLS-1$
					.append(" from ").append(ISysObject.TYPE_DM_SYSOBJECT).append(" where folder ( id ( '") //$NON-NLS-1$ //$NON-NLS-2$
					.append(folderModel.getObjectId().getId()).append(
				...
				ResultView.getInstance().addResultSet(
					...
					"Folder content", false, query.getDQL(), true); //$NON-NLS-1$
