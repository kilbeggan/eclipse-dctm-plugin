...
/**
 * <H4>Action triggered when a table is selected.</H4>
 * <DL>
 * <DT><B>Copyright (c): </B>
 * <DD>Christopher Harper</DD>
 * </DT>
 * <DT><B>Created : </B>
 * <DD>Nov 28, 2004 9:37:35 AM.</DD>
 * </DT>
 * </DL>
 * <p>
 ...
 * @since 1.0
 */
public class SelectTableAction
								extends
									AbstractBaseAction

{
	...
	/**
	 * Query for the table details and place that information into the results
	 * view.
	 * <DL>
	 * <DT><B>Created : </B>
	 * <DD>Nov 28, 2004 9:39:31 AM</DD>
	 * </DT>
	 * <DT><B>Author : </B>
	 * <DD>Christopher Harper account : HARPECHR</DD>
	 * </DT>
	...
	public void run()
	{
		...
		final Model selectedModel = ConnectionView.getInstance()
			.getSelectedModel();
		if(selectedModel instanceof TableModel)
		{
			final TableModel tableModel = (TableModel) selectedModel;
			...
			try
			{
				...
				if(tableName.indexOf(".") > -1) //$NON-NLS-1$
				{
					tableName = tableName.substring(tableName.indexOf(".") + 1); //$NON-NLS-1$
				}

				connection = connectionModel.getSession();
				final StringBuffer attributeDQL = new StringBuffer(
					"select distinct ").append(IRegistered.COLUMN_NAME) //$NON-NLS-1$
					.append(" , ").append(IRegistered.COLUMN_DATATYPE).append(" , ").append(IRegistered.COLUMN_LENGTH) //$NON-NLS-1$ //$NON-NLS-2$
					.append(" from ").append(IRegistered.TYPE_DM_REGISTERED).append(" where ") //$NON-NLS-1$ //$NON-NLS-2$
					.append(IRegistered.TABLE_NAME)
					.append(" = '").append(tableName).append("' order by 1"); //$NON-NLS-1$ //$NON-NLS-2$
